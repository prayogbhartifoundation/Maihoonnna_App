...
router.post('/', async (req, res) => {
    const { beneficiaryId, careCompanionId, scheduledTime, durationMinutes, benefitId } = req.body;
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (benefitId) {
                ...
                const balance = subscription.balances.find(b => b.benefitId === benefitId);
                if (!balance) {
                ...
                if (balance.totalUnits !== -1 && (balance.totalUnits - balance.usedUnits) < unitsNeeded) {
