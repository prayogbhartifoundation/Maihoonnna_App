            ...
            select: {
                ...
                subscriberBeneficiaries: {
                    select: {
                        ...
                        subscriptions: {
                            where: { isActive: true },
                            select: { id: true, packageType: true, startDate: true, endDate: true },
                            take: 1,
                        },
                    },
                },
...
router.post('/admin-enroll', async (req, res) => {
    ...
    if (!subscriberPhone || !subscriberName || !packageId) {
        return res.status(400).json({ success: false, message: 'subscriberPhone, subscriberName, and packageId are required' });
    }

    // If beneficiary is different, we need their phone
    if (!sameAsSubscriber && !beneficiaryPhone) {
        return res.status(400).json({ success: false, message: 'beneficiaryPhone is required when sameAsSubscriber is false' });
    }
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            // ──────────────────────────────────────────────────────────────────
            let subscriberUser = await tx.user.findUnique({ where: { phone: subscriberPhone } });
            if (!subscriberUser) {
                subscriberUser = await tx.user.create({
                    data: {
                        ...
                        password: dummyHash,
                        isActive: true,
                        location: subscriberAddress ? `${subscriberAddress}, ${subscriberCity || ''}, ${subscriberState || ''} - ${subscriberPincode || ''}`.trim() : '',
                    },
                });
            } else {
                // Update name if provided and different
                const newLocation = subscriberAddress ? `${subscriberAddress}, ${subscriberCity || ''}, ${subscriberState || ''} - ${subscriberPincode || ''}`.trim() : subscriberUser.location;
                if (subscriberUser.name !== subscriberName || subscriberUser.location !== newLocation) {
            ...
            if (sameAsSubscriber) {
                ...
                beneficiaryUser = await tx.user.findUnique({ where: { phone: benPhone } });
                if (!beneficiaryUser) {
            ...
            // ──────────────────────────────────────────────────────────────────
            let beneficiary = await tx.beneficiary.findUnique({ where: { userId: beneficiaryUser.id } });
            if (!beneficiary) {
                        ...
                        emergencyContacts: {
                            create: [
                                ...(emergencyContactName ? [{
                                    ...
                                    phone: emergencyContactPhone || subscriberPhone.replace('+91', ''),
                ...
                if (medicalConditions && medicalConditions.length > 0) {
                    for (const conditionName of medicalConditions) {
                        const slug = conditionName.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '');
                        // Find or create the condition
                        let cond = await tx.medicalCondition.findUnique({ where: { name: conditionName } });
                        if (!cond) {
            ...
            return {
                subscription: sub,
                subscriber: { id: subscriberUser.id, name: subscriberUser.name, phone: subscriberUser.phone },
