...
router.get('/my-team', async (req, res) => {
    try {
        ...
        if (isFM) {
            ...
            console.log(`[FM Portal] User: ${req.user.id}, FM Profile: ${fmProfileId}`);
            
            if (!fmProfileId) {
                return res.status(404).json({ success: false, message: 'Field Manager profile not found' });
            }
        ...
        todayVisits.forEach(v => {
            if (!visitMapByCC[v.careCompanionId]) visitMapByCC[v.careCompanionId] = [];
        ...
        const mapped = companions.map(cc => ({
            ...
            activeVisitCount: (visitMapByCC[cc.id] || []).filter(v => v.status === 'in_progress').length,
...
router.get('/my-team/schedule', async (req, res) => {
    try {
        ...
        const visits = await prisma.visit.findMany({
            where: visitWhere,
            include: {
                careCompanion: { select: { id: true, name: true, zone: true, isAvailable: true } },
                beneficiary: { select: { id: true, name: true, address: true, pincode: true } }
            },
    ...
    try {
        ...
        if (req.user?.role === 'field_manager') {
            ...
            const ben = await prisma.beneficiary.findUnique({
                where: { id },
                select: { fieldManagerId: true, primaryCcId: true, secondaryCcId: true }
            });
            ...
            if (!hasAccess) {
                return res.status(403).json({ success: false, message: 'Access denied to this beneficiary' });
            }
        }
        ...
        const subscription = await prisma.subscription.findFirst({
            ...
            include: {
                ...
                benefitBalances: {
                    include: {
                        benefit: {
                            select: { id: true, name: true, unitLabel: true, description: true }
                        }
                    }
                }
            },
        ...
        if (!subscription) {
            return res.json({ success: true, data: { subscription: null, benefitBalances: [], hoursLog: [] } });
        }
        ...
        const benefitBalances = subscription.benefitBalances.map(b => ({
            ...
            usagePercent: b.totalUnits > 0 ? Math.round((b.usedUnits / b.totalUnits) * 100) : 0,
        }));
        ...
        console.error('GET /field-manager/beneficiaries/:id/benefit-usage error:', err);
        res.status(500).json({ success: false, message: err.message });
    }
});
