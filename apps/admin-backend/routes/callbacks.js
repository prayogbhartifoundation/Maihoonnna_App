...
router.get('/', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: 'Failed to fetch callback requests' });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: 'Failed to submit callback request' });
  }
});
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: 'Failed to update callback request' });
  }
});
