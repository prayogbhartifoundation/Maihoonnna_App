 ...
 */
async function checkCCAvailability(careCompanionId, startTime, durationMinutes) {
    ...
    if (config) {
        const { globalLunchStart, globalLunchEnd } = config;
        ...
        if (startTime < lunchEndToday && endTime > lunchStartToday) {
            return { isAvailable: false, reason: 'Conflict with global lunch break (1 PM - 2 PM)' };
        }
    }
    ...
    if (!cc?.isAvailable) {
        return { isAvailable: false, reason: 'Care Companion is marked as unavailable' };
    }
    ...
    if (blockers.length > 0) {
        return { isAvailable: false, reason: 'Conflict with manual blocker: ' + blockers[0].reason };
    }

    // 4. Check Existing Visits with 30-min Buffer
    // A visit at [vStart, vEnd] creates a blocked window of [vStart - 30m, vEnd + 30m]
    const searchStart = new Date(startTime.getTime() - BUFFER_MS - (durationMinutes * 60000)); 
    const searchEnd = new Date(endTime.getTime() + BUFFER_MS + (durationMinutes * 60000));
    ...
    for (const visit of existingVisits) {
        ...
        if (startTime < blockedEnd && endTime > blockedStart) {
            return { isAvailable: false, reason: `Visit conflict (includes 30m travel buffer)` };
        }
    }
